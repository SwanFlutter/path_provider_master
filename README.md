...
[![pub package](https://img.shields.io/pub/v/path_provider_master.svg)](https://pub.dev/packages/path_provider_master)
[![Pub Points](https://img.shields.io/pub/points/path_provider_master)](https://pub.dev/packages/path_provider_master/score)
[![Popularity](https://img.shields.io/pub/popularity/path_provider_master)](https://pub.dev/packages/path_provider_master)
[![Pub Likes](https://img.shields.io/pub/likes/path_provider_master)](https://pub.dev/packages/path_provider_master)
[![GitHub issues](https://img.shields.io/github/issues/SwanFlutter/path_provider_master)](https://github.com/SwanFlutter/path_provider_master/issues)
[![GitHub forks](https://img.shields.io/github/forks/SwanFlutter/path_provider_master)](https://github.com/SwanFlutter/path_provider_master/network/members)
...
// dart:io is only needed when working directly with File/Directory on native platforms
import 'dart:io';
```
...
```dart
// Good practice
final result = await PathProviderMaster.getPublicPicturesDirectory();
if (result != null) {
  final dir = Directory(result.path);
  if (!await dir.exists()) {
    await dir.create(recursive: true);
  }
...
```dart
final result = await PathProviderMaster.getApplicationDocumentsDirectory();
if (result != null) {
  final dir = Directory(result.path);
  if (!await dir.exists()) {
    await dir.create(recursive: true);
...
**Q: CMake error on Windows — "Compatibility with CMake < 3.5 has been removed"**

This error appears when building on Windows with CMake 4.x:
```
CMake Error at googletest-src/CMakeLists.txt:4 (cmake_minimum_required):
  Compatibility with CMake < 3.5 has been removed from CMake.
```
This is caused by an older version of GoogleTest (1.12.1) that is incompatible with CMake 4.x.

**Fix:** Make sure you are using version `1.0.3` or later of this package, which upgrades GoogleTest to v1.14.0 and resolves this issue.

If you are using an older version or building from source, update `windows/CMakeLists.txt` and change the GoogleTest URL:
```cmake
# Change this line:
...
```
Then delete the `example/build` folder and rebuild.
